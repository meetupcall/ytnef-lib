...
pkgdatadir = $(datadir)/libytnef
pkglibdir = $(libdir)/libytnef
...
nobase_include_HEADERS = ytnef.h \
...
CONFIG_CLEAN_FILES = libytnef.spec
LTLIBRARIES = $(lib_LTLIBRARIES)

libytnef_la_LDFLAGS =
libytnef_la_LIBADD =
am_libytnef_la_OBJECTS = ytnef.lo
libytnef_la_OBJECTS = $(am_libytnef_la_OBJECTS)
...
CFLAGS = -g -O2
DIST_SOURCES = $(libytnef_la_SOURCES)
HEADERS = $(nobase_include_HEADERS)

DIST_COMMON = README $(nobase_include_HEADERS) AUTHORS COPYING \
	ChangeLog INSTALL Makefile.am Makefile.in NEWS aclocal.m4 \
	config.guess config.h.in config.sub configure configure.ac \
	depcomp install-sh libytnef.spec.in ltmain.sh missing \
	mkinstalldirs
SOURCES = $(libytnef_la_SOURCES)
...
libytnef.spec: $(top_builddir)/config.status libytnef.spec.in
...
libytnef.la: $(libytnef_la_OBJECTS) $(libytnef_la_DEPENDENCIES) 
	$(LINK) -rpath $(libdir) $(libytnef_la_LDFLAGS) $(libytnef_la_OBJECTS) $(libytnef_la_LIBADD) $(LIBS)
